...
def evidence_chain(evidence_graph: EvidenceGraph, finding_id: str) -> dict[str, Any]:
    ...
    for _, audit_id, data in graph.out_edges(finding_id, data=True):
        if data.get("relation") != schema.REL_CITES:
            continue
        if audit_id not in graph or graph.nodes[audit_id].get("type") != schema.NODE_TOOL_EXECUTION:
            gaps.append({"from": finding_id, "missing": audit_id, "reason": "audit_id not found"})
            continue
...
def host_summary(evidence_graph: EvidenceGraph, hostname: str = "") -> dict[str, Any]:
    ...
    return {"hosts": [_single_host_summary(graph, h) for h in hosts], "total_hosts": len(hosts)}
...
def _single_host_summary(graph: nx.DiGraph, hostname: str) -> dict[str, Any]:
    ...
    timestamps: list[str] = []
    for src, _, data in graph.in_edges(host_id, data=True):
...
def _hop(
    ...
    return {
        ...
        "relation": relation,
        "to": dst,
        "node_type": graph.nodes[dst].get("type", "unknown") if dst in graph else "missing",
...
def _timestamp(data: dict[str, Any]) -> str:
    return str(data.get("event_timestamp") or data.get("timestamp") or data.get("ts") or "")
