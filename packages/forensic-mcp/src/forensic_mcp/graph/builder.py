...
def _load_evidence(
    ...
    for idx, evidence in enumerate(_as_list(records)):
        if not isinstance(evidence, dict):
            continue
        ev_id = str(
            evidence.get("id") or evidence.get("evidence_id") or f"E-{idx + 1:03d}"
        )
        graph.add_node(
            ev_id, type=schema.NODE_EVIDENCE, **schema.evidence_props(evidence)
        )
...
def _load_audit_entries(
    ...
    for path in sorted(audit_dir.glob("*.jsonl")):
        ...
        for line in lines:
            ...
            graph.add_node(
                str(audit_id),
                type=schema.NODE_TOOL_EXECUTION,
                **schema.audit_props(entry),
            )
    return entries
    ...
    for entry in audit_entries:
        for input_file in entry.get("input_files", []) or []:
            ...
            if producer and producer != entry["audit_id"]:
                graph.add_edge(
                    entry["audit_id"], producer, relation=schema.REL_OUTPUT_OF
                )
