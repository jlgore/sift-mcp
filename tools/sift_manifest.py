...
RESOLUTION_MAP: dict[str, list[str]] = {
    ...
    "sleuthkit": [
        "fls", "mmls", "icat", "istat", "blkls", "blkcat", "blkstat", "blkcalc",
        "fsstat", "ils", "ffind", "ifind", "fcat", "jcat", "jls", "img_stat",
        "img_cat", "mactime", "tsk_recover", "tsk_gettimes", "tsk_loaddb",
        "tsk_comparedir", "sigfind", "sorter", "hfind", "usnjls", "srch_strings",
    ],
    "plaso-tools": ["log2timeline.py", "psort.py", "pinfo.py", "psteal.py", "image_export.py"],
    "python3-plaso": ["log2timeline.py", "psort.py", "pinfo.py", "psteal.py", "image_export.py"],
    "bulk-extractor": ["bulk_extractor"],
    "afflib-tools": ["affcat", "affconvert", "affcopy", "affinfo", "affverify", "affuse"],
    "pst-utils": ["readpst", "lspst", "pst2ldif", "pst2dii"],
    "hashdeep": ["hashdeep", "md5deep", "sha1deep", "sha256deep"],
    "ntfs-3g": ["ntfscat", "ntfsls", "ntfsinfo", "ntfscluster", "ntfsundelete", "ntfsfix"],
...
NOISE_PACKAGES = {
    # build tooling
    "build-essential", "gcc", "g++", "swig", "flex", "pkg-config", "patch",
    "cpanminus", "make",
    # python build/runtime deps
    "python3-pip", "python3-setuptools", "python3-setuptools-rust",
    "python3-wheel", "python3-virtualenv", "python3-dev", "python3-tk",
    "python3-pyqt5", "python3-wxgtk4.0", "python3-m2crypto",
    "python3-keyrings.alt", "python3-redis", "python3-xlsxwriter", "python3",
    "python-is-python3", "ipython3", "python3-keyring",
    # generic runtimes / JVM / .NET host
    "default-jre", "openjdk-8-jdk", "openjdk", "libbcprov-java",
    "libcommons-lang3-java", "dotnet-sdk-9.0", "dotnet", "powershell",
    "perl", "tcl", "blt",
    # desktop / system apps (not evidence analysis)
    "chromium-browser", "wine", "wine-stable", "unity-control-center",
    "onboard", "orca", "okular", "feh", "gthumb", "transmission", "samba",
    "winbind", "docker", "docker-ce", "qemu", "qemu-system", "lvm2", "mdadm",
    "apache2", "virtuoso-minimal", "phonon", "phonon4qt5", "zenity", "dbus-x11",
    "xdot", "magnus", "bless", "ghex", "kdiff3", "vim", "hexedit", "htop",
    "git", "curl", "vbindiff", "graphviz", "dconf-cli", "software-properties-common",
    "android-sdk-platform-tools", "aws-cli", "claude-code", "at", "net-tools",
    "open-iscsi", "nbd-client", "cifs-utils", "exfat-extras", "e2fsprogs",
    "xfsprogs", "squashfs-tools", "cryptsetup", "ntfs-3g",  # ntfs-3g: mount helpers; kept via map only for read tools
}
NOISE_PATTERNS = [
    re.compile(r"-dev$"),         # headers
    re.compile(r"-perl$"),        # perl library modules
    re.compile(r"^libgtk"), re.compile(r"^libcairo"), re.compile(r"^libgirepository"),
    re.compile(r"^libssl"), re.compile(r"^libbz2"), re.compile(r"^libffi"),
    re.compile(r"^libxml2"), re.compile(r"^libxslt"), re.compile(r"^libncurses"),
    re.compile(r"^libasound"), re.compile(r"^libpcap"), re.compile(r"^libnet1"),
    re.compile(r"^libglib"), re.compile(r"^libicu"), re.compile(r"^zlib"),
    re.compile(r"^libext2fs"), re.compile(r"^libfuse"),
]
# Pure library packages (no CLI we want) that don't match the patterns above.
NOISE_LIBS = {
    "libafflib", "libbde", "libesedb", "libevt", "libevtx", "libewf",
    "libewf2", "libewf-python3", "libfvde", "libmsiecf", "libolecf",
    "libpff", "libregf", "libregf-python3", "libvshadow", "libvshadow-python3",
    "libvmdk", "libvhdi", "liblightgrep", "libyara3", "python3-yara",
    "python3-pypff", "python3-pytsk3", "python3-tsk", "pytsk3", "python3-dfvfs",
    "python3-fuse", "python3-debian", "python3-magic", "python3-pefile",
    "libdatetime-perl", "libencode-perl", "libtext-csv-perl",
...
NETWORK_OFFLINE = {  # parse captures from disk; safe under --unshare-net
    "tcpflow", "tcptrace", "ngrep", "nfdump", "nfcapd", "nfanon", "tcpxtract",
    "tcpick", "tcpstat", "tcpprof", "tcpslice", "tcptrack", "tcpreplay",
    "ssldump", "p0f", "ngrep",
}
LIVE_NETWORK = {  # need live network; out of scope for evidence analysis
    "hydra", "nikto", "aircrack-ng", "ettercap", "dsniff", "driftnet",
    "arp-scan", "nbtscan", "netwox", "sslsniff", "etherape", "netsed",
}
...
def commands_from_usrbin(text: str, jinja: dict[str, list[str]]) -> set[str]:
    ...
    cmds: set[str] = {
        c for c in USRBIN_LITERAL_RE.findall(text)
...
def parse_sls(path: Path):
    ...
    if path.parent.name == "packages":
        ...
        for state_id, named in pkg_blocks:
            if not named and state_id and "{" not in state_id and not state_id.startswith("sift-"):
                apt.add(state_id)
...
def ensure_saltstack(path: Path) -> Path:
    ...
    subprocess.run(["git", "clone", "--depth", "1", SALTSTACK_URL, str(path)], check=True)
    return sift
...
def build_manifest(args) -> tuple[list[dict], dict]:
    ...
    if args.refresh_ground_truth:
        gt = refresh_ground_truth(args.dpkg_host, sorted(apt_packages), args.ground_truth)
    elif args.dpkg_host:
        gt = refresh_ground_truth(args.dpkg_host, sorted(apt_packages), args.ground_truth)
...
def _confirmed_by_gt(cmds, gt) -> bool:
    allbins = {b for v in gt.values() if v.get("installed") for b in v.get("binaries", [])}
...
def write_outputs(records, summary, out_dir: Path):
    ...
    lines.append(f"| B | FK knowledge exists, **no catalog entry** (enrichment win) | {len(buckets['B'])} |")
    lines.append(f"| C | neither — long tail, runnable via run_command | {len(buckets['C'])} |")
    lines.append(f"|   | &nbsp;&nbsp;of which already in catalog but unenriched | {len(cataloged_unenriched)} |")
    lines.append("")
    ...
    if buckets["B"]:
        ...
        for r in sorted(buckets["B"], key=lambda r: r["command"]):
            lines.append(f"| `{r['command']}` | {r['package']} | {r['install_source']} | {r['fk_tool']} |")
    else:
        lines.append("_None — every FK tool present in SIFT is already cataloged._")
    lines.append("")

    lines.append("## A — covered (catalog + FK)\n")
    lines.append(", ".join(f"`{r['command']}`" for r in sorted(buckets["A"], key=lambda r: r["command"])) or "_none_")
    lines.append("")

    lines.append("## C — long tail (unenriched, runnable)\n")
    if cataloged_unenriched:
        lines.append("Cataloged but no FK enrichment: " +
                     ", ".join(f"`{r['command']}`" for r in sorted(cataloged_unenriched, key=lambda r: r["command"])))
        lines.append("")
    lines.append("Not in catalog, no FK (sample of the long tail):")
    tail = [r for r in buckets["C"] if not r["in_catalog"]]
    lines.append(", ".join(f"`{r['command']}`" for r in sorted(tail, key=lambda r: r["command"])[:80]))
    lines.append("")
    ...
    lines.append("## Notes\n")
    lines.append("- The SIFT *server* image installs the full tool set; the desktop "
                 "image adds only config, so `desktop_only` is `false` throughout.")
    lines.append("- Command names are ground-truthed from `dpkg -L` on a live SIFT box "
                 "where available; `inferred: true` marks names resolved from the "
                 "curated map or package name and listed under needs_review.")
    lines.append("- Denied binaries are never emitted: " +
...
def main():
    ...
    ap.add_argument("--saltstack-dir", type=Path, default=DEFAULT_SALTSTACK,
    ...
    ap.add_argument("--dpkg-host", default=None,
                    help="ssh host (user@host) of a live SIFT box for dpkg grounding")
    ap.add_argument("--refresh-ground-truth", action="store_true",
    ...
    print(f"  FK tools in SIFT:    {fk_present}  ({len(buckets['A'])} cataloged = {rate}% FK-covered)")
    print(f"  A / B / C:           {len(buckets['A'])} / {len(buckets['B'])} / {len(buckets['C'])}")
